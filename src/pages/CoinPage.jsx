...
const CoinPage = () => {
  ...
  const API_URL = `https://api.coingecko.com/api/v3/coins/${params.coinId}?localization=false&sparkline=true`;
  useEffect(() => {
        ...
        <div>
          <div className="flex justify-between">
            {coin.market_data?.current_price ? (
              <p className="text-3xl font-bold">
                R {coin.market_data.current_price.zar.toLocaleString()}
              </p>
            ) : null}
            <p>7 day</p>
          </div>
          ...
          <div className="flex justify-between py-4">
            ...
            <div>
              ...
              {coin.market_data?.market_cap ? (
                <p>R{coin.market_data.total_volume.zar.toLocaleString()}</p>
              ) : null}
            </div>
          </div>
          ...
          <div className="flex justify-between py-4">
            <div>
              ...
              {coin.market_data?.high_24h ? (
                <p>R{coin.market_data.high_24h.zar.toLocaleString()}</p>
              ) : null}
            </div>
            <div>
              <p className="text-gray-500 text-sm">24h Low</p>
              {coin.market_data?.low_24h ? (
                <p>R{coin.market_data.low_24h.zar.toLocaleString()}</p>
              ) : null}
            </div>
          </div>
        </div>
